...
/**
 * Cached, not per-request. Every fan browsing this event sees the same listings,
 * so rendering it fresh for each one buys nothing — and this is the highest
 * traffic surface in the app. Checkout stays `force-dynamic`; browsing does not
 * need to be, and knowing which is which is most of what caching strategy is.
 */
export const revalidate = 30;

/**
 * Prerender the catalogue at build time so an event page is a static file hit.
 *
 * With a handful of events, prerendering all of them is free. A real catalogue
 * would generate the top few thousand by demand and leave the long tail to be
 * rendered on first request and cached from then on — which is what
 * `dynamicParams` (true by default) already does for anything not listed here.
 */
...
/**
 * Picking a listing creates the checkout session. It is a form POST to a Server
 * Action, so the session — and therefore the resumable URL — exists before any
...
export default async function EventPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-5 py-10">
      ...
      {/* `useSearchParams` needs a boundary in a cached route: the shell is
