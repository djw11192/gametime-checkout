...
export function buttonClass(
  ...
  return cn(
    ...
    // `<button>` shows an arrow cursor by default, so the pointer has to be
    // asked for. The `:disabled` rule wins over it, so a blocked button still
    // looks blocked.
...
/** A loading placeholder. Callers set the size, so it can match what replaces it. */
